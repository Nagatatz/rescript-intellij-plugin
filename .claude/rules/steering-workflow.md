...
**例外:** ステアリングを省略してよいケースは → `.claude/rules/definition-of-done.md` の「例外（フェーズ横断の免除マトリクス）」の「ステアリング省略」列を参照すること（定量基準: 3ファイル以下 / 50行以下 / 新規クラスなし / 新規EPなし / public API不変。典型例: タイポ修正・1行の設定変更・コメント追加・定数値修正）。
