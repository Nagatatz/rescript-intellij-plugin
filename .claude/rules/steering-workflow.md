...
## 調査・リサーチタスクのステアリングとコミット

**以下は強制的な行動指示であり、例外なく従うこと。**

コードの変更を伴わない調査・リサーチタスク（機能調査、技術調査、競合分析、ギャップ分析等）であっても、ステアリングドキュメントを作成し、完了後にコミットすること。

**対象:**
- 機能候補の調査・優先度分析
- 技術的な実現可能性調査
- 他プラグイン・他言語との比較分析
- UX 優先度分析・ユーザージャーニー分析
- アーキテクチャ調査・設計検討

**手順:**
1. `.steering/[YYYYMMDD]-[NNN]-[調査タイトル]/` ディレクトリを作成する
2. 調査成果物（`research.md`, `report.md`, `analysis.md` 等）を作成する
3. 調査完了後、`main` ブランチに直接コミットする（コード変更を伴わないため worktree・feature ブランチは不要）
4. コミットメッセージは `📝 Add <調査内容の簡潔な説明>` とする

**理由:** 調査成果物はプロジェクトの意思決定の根拠であり、将来の実装計画や優先度判断の参照資料となる。コミットせずに放置すると、知識が散逸し、同じ調査を繰り返すリスクがある。
