...
| **Watch** | `rescript watch` | Continuous compilation with file watching |
...
The **Watch** command starts the compiler in watch mode, which monitors your source files for changes and automatically recompiles them. ReScript 12.0 promoted `rescript watch` to a first-class subcommand and deprecated the older `rescript build -w` flag — the plugin invokes the new form so the compiler does not emit deprecation warnings.
...
:::{tip}
You can keep a Watch run configuration running alongside your editor session. Compilation errors will appear both in the Run panel console and as LSP diagnostics (red underlines) in the editor.
:::
...
| `rescript watch` | Compile in watch mode |
