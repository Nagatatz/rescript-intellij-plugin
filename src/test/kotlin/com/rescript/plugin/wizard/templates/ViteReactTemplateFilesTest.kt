...
class ViteReactTemplateFilesTest {
    ...
    fun `zod variant ships Validation res and pins the zod dependency`() {
        ...
        val files = ViteReactTemplateFiles.generate(zodCtx)
        assertTrue(files["src/Validation.res"]!!.contains("@module(\"zod\")"))
        assertTrue(files["src/Validation.res"]!!.contains("parseGreetForm"))
    ...
    fun `sury variant ships Validation res and pins the sury dependency`() {
        ...
        val files = ViteReactTemplateFiles.generate(suryCtx)
    ...
    @Test
    fun `App res validates form input through Validation parseGreetForm`() {
        val app = ViteReactTemplateFiles.generate(ctx)["src/App.res"]!!
        assertTrue(app.contains("Validation.parseGreetForm"))
    }
}
