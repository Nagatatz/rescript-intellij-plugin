...
object RescriptTokenTypes {
    ...
    val KEYWORDS: TokenSet = TokenSet.create(
        ASYNC, AWAIT, AND, AS, ASSERT, BEGIN, CATCH, CLASS, CONSTRAINT,
        DO, DONE, DOWNTO, ELSE, END, EXCEPTION, EXTERNAL, FOR, FUNCTOR,
        IF, IN, INCLUDE, INHERIT, INITIALIZER, LAZY, LET, LIST, MODULE,
        MUTABLE, NEW, NONREC, OBJECT, OF, OPEN, OR, PUB, PRI, RAW, REC,
        SIG, STRUCT, SWITCH, THEN, TRY, TYPE, UNPACK, VAL, VIRTUAL,
        WHEN, WHILE, WITH, UNIT, REF, RAISE, METHOD, PRIVATE, MATCH,
    )
    ...
    val OPERATORS: TokenSet = TokenSet.create(
        STRING_CONCAT, ARROW, RIGHT_ARROW, LEFT_ARROW, PIPE_FORWARD,
        EQEQEQ, EQEQ, EQ, NOT_EQEQ, NOT_EQ, COLON_EQ, COLON_GT,
        LT_OR_EQUAL, L_AND, L_OR, MOD, LAND, LOR, LXOR, LSL, LSR, ASR,
        PLUS, MINUS, STAR, SLASH, PERCENT, CARRET,
        PLUSDOT, MINUSDOT, STARDOT, SLASHDOT,
    )

    /** Top-level declaration keywords (used by the lightweight parser). */
    @JvmField
    val TOP_LEVEL_KEYWORDS: TokenSet = TokenSet.create(
        LET, TYPE, MODULE, EXTERNAL, OPEN, INCLUDE, EXCEPTION,
    )
}
