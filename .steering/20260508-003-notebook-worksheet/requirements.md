...
- [ ] `.resnb` ファイルが「ReScript Notebook」として認識される（FileType + Icon）
- [ ] ファイル形式は JSON で、`{ "version": 1, "cells": [{ "code": "...", "lastOutput": "..." }] }` のスキーマを持つ
- [ ] 空の `.resnb` ファイルは `{ "version": 1, "cells": [] }` として扱われる
- [ ] 不正な JSON は安全にエラー表示し、ユーザーがファイル全体を失わない（パース失敗時は raw text view にフォールバック）
...
- [ ] `.resnb` を開くと専用の `FileEditor` が起動し、セルが縦に並んで表示される
- [ ] 各セルには「コード入力エリア（multi-line text area）」「Run ボタン」「結果表示エリア（read-only）」がある
- [ ] ツールバーに「Add Cell」「Run All」ボタンがある
- [ ] 各セルの右上に「Delete」「Move Up」「Move Down」ボタンがある
- [ ] セル数が 0 のとき、自動的に空セル 1 つを追加する
- [ ] 編集内容は通常の Save (`Ctrl+S`) でファイルに JSON 形式で保存される
...
- [ ] Run ボタンを押すと既存の `RescriptReplExecutor` 経由でセルコードが評価される
- [ ] 評価結果（stdout + stderr）が結果エリアに表示される
- [ ] コンパイルエラーは赤、成功時の出力は通常色で表示する
- [ ] 評価中はボタンが「Running…」に変わり、ダブル実行を防ぐ
- [ ] 評価結果は cell.lastOutput として保存される（次回ファイルを開いても結果が残る）
...
- [ ] ツールバーに「Export Markdown」ボタンがあり、現在の Notebook を Markdown 文字列にしてクリップボードにコピーできる
- [ ] Markdown 形式: 各セルを ` ```rescript ... ``` ` でコードブロック、その下に ` ``` ... ``` ` で出力ブロックを並べる
...
- [ ] 空 Notebook を開いて 1 セル追加・評価・保存・再オープンが動作する
- [ ] 3 セルの Notebook を Markdown エクスポートして外部に貼れる
- [ ] 不正な JSON ファイルを開いてもクラッシュしない
- [ ] ユニットテストで JSON シリアライザ・パーサと Markdown エクスポータをスナップショット検証する
