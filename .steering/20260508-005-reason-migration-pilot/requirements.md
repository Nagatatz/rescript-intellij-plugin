...
- [ ] `Tools > Show Reason Migration Pilot` メニューから ToolWindow を開ける
- [ ] ToolWindow には現在のプロジェクト内の `.re` / `.rei` ファイルが一覧表示される
- [ ] 各エントリには `path/to/file.re` 形式のパスが表示される
- [ ] チェックボックスで変換対象を選択できる
- [ ] 「Select All」「Clear」ボタンで一括選択／解除できる
- [ ] Refresh ボタンで再スキャンできる
- [ ] ファイルが見つからない場合は「No Reason files found」プレースホルダーを表示する
...
- [ ] ツールバーに「Convert Selected」ボタンがあり、チェックされたファイルを順次 `rescript convert` で変換する
- [ ] 各変換は `ProcessBuilder` でサブプロセスとして実行される（`ProcessBuilder` の引数リストは `RescriptSecurityUtils` が生成する安全な配列を渡す）
- [ ] 変換成功時: 元の `.re` ファイルを `.res` にリネーム + 内容を変換結果で置き換え
- [ ] 変換失敗時: エラーメッセージを結果領域に表示し、元ファイルは保持する
- [ ] 進行状況をプログレスバー or ステータスバーに表示する（`X/Y converted`）
- [ ] Cancel ボタンで残りの変換を中止できる
...
- [ ] 変換完了時にサマリーダイアログまたは ToolWindow の結果領域に「N succeeded, M failed」を表示する
- [ ] 失敗したファイルは結果リストにエラーメッセージ付きで残る
- [ ] サマリーは `Copy` ボタンでクリップボードにコピーできる（PR 説明等に貼れる）
...
- バッチサイズの自動調整・並列変換（Phase 2 以降）
- 変換中の差分プレビュー（Phase 2 以降、ファイル単位の確認は IDE の Local History に任せる）

## 受け入れ確認

- [ ] サンプル `.re` ファイル 3 つを選択して一括変換できる — マージ後手動検証
- [ ] 不正な `.re` ファイル（構文エラー）でも他のファイルの変換が継続される — マージ後手動検証
- [ ] `rescript` CLI が見つからない場合、「rescript binary not found」エラーが表示される
- [ ] ユニットテストでファイル列挙ロジックと変換結果パースをスナップショット検証する
