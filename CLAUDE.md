...
Plan（ステアリングワークフロー）を必要とする非自明な実装を並列で行う場合、**必ず git worktree で専用の隔離空間を作成し、そこで実装すること**。同一ワーキングツリー内で並列に実装作業を行うと、コミットやブランチ操作でコンフリクトが発生するため禁止する。

- 軽微な修正（タイポ修正、1行変更等）は git worktree 不要
- ステアリングワークフローを伴う実装作業は、**たとえ1機能のみの場合でも**、メインリポジトリと別の Claude Code ウィンドウで並行作業する場合は git worktree を使用すること
