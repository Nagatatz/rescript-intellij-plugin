...
| `wizard/templates/` | 17 種類のプロジェクトテンプレートファイル生成（全テンプレートが zod/sury の `Validation.res` を variants/<key>/ から選択） | `BasicTemplateFiles`, `ViteReactTemplateFiles`, `HonoInertiaTemplateFiles` 等 |
