...
| `wizard/templates/` | 21 種類のプロジェクトテンプレートファイル生成（既存 17 件は zod/sury の `Validation.res` を variants/<key>/ から選択。新 4 件 TanStack Start / Remix RR v7 / Astro / Waku は Validation 選択を無効化） | `BasicTemplateFiles`, `ViteReactTemplateFiles`, `HonoInertiaTemplateFiles`, `TanstackStartTemplateFiles`, `RemixV7TemplateFiles`, `AstroTemplateFiles`, `WakuTemplateFiles` 等 |
