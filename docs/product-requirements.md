...
3回の機能調査（初回調査・追加調査・関数型言語調査）で109件の未実装機能候補を収集し、74件は実装済み（S/A/B 優先度42件 + Phase 1 Quick Wins 7件 + S 優先度6件 + A 優先度13件 + B 優先度6件）。#79 (MultiLang Commenter) は ReScript/JS のコメント構文が同一のため不要と判断。残り35件を B/C の2段階で優先度付けする。
