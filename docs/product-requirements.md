...
- [x] 20 種類のテンプレート（Basic / Vite+React / Next.js / Hono / Hono GraphQL / Cloudflare Workers / AWS Lambda / Google Cloud Run / Electron / React Native (Expo) / React Native (CLI) / npm Library / CLI Tool / Monorepo / Full-Stack / ReS-X / TanStack Start / Remix RR v7 / Astro / Waku）が選択できる
...
- [x] 既存 16 テンプレートでは用途に合わせた `Validation.res` が `variants/<key>/` から書き出される
