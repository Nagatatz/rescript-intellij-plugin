...
- **Type coverage heat map** — Tool window (`View > Tool Windows > ReScript Type Coverage`) that lists every `.res` file with its annotated / inferred / total `let` counts and a colour-coded coverage percentage; sorted ascending so files in need of explicit types float to the top, double-click to jump. A toolbar action batch-inserts inferred type annotations into the selected file's un-annotated top-level `let`s in one undoable step
...
- **JSX paired-tag rename** — Editing a JSX tag name live-mirrors the change onto its matching opening/closing tag, undoable in one step
