...
| **Rename Variant Constructor** (intention) | `main/src/VariantSamples.res` + `VariantUsage.res` | Caret on `Loading` (in either file) → `Alt+Enter` → "Rename variant constructor". Confirm dialog shows 4 occurrences across 2 files; undo with one `Ctrl+Z`. `VariantUsage.res` is a *cross-file fixture* — it only re-uses constructors declared in `VariantSamples.res` so the rename can prove it touches multiple files. Open it on its own and nothing special happens; there is no dedicated tool window for it. |
